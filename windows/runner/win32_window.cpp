...
LRESULT CALLBACK Win32Window::WndProc(HWND const window,
	...
	if (message == WM_NCCREATE) {
		...
		//std::ofstream log_file("shutdown_log.txt", std::ios::app); // Open file in append mode
		//if (log_file.is_open()) {
		//	log_file << "！！！！！！" << std::endl; // Write to file
		//	log_file.close(); // Close the file
		//}
		return TRUE;
	}
	else if (message == WM_ENDSESSION) {
		if (x::message_api != nullptr && wparam) {
			// Create a promise and future pair
			//std::promise<void> shutdown_promise;
			//std::future<void> shutdown_future = shutdown_promise.get_future();
			//std::ofstream log_file("shutdown_log.txt", std::ios::app); // Open file in append mode
			//if (log_file.is_open()) {
			//	log_file << "111111111111111111111" << std::endl; // Write to file
			//	log_file.close(); // Close the file
			//}
			x::message_api->NotifyShutdown(
				[]() {
					//std::ofstream log_file("shutdown_log.txt", std::ios::app); // Open file in append mode
					//if (log_file.is_open()) {
					//	log_file << "Notify Shutdown succeeded!" << std::endl; // Write to file
					//	log_file.close(); // Close the file
					//}
					// // This is the on_success callback
					 std::cout << "Notify Shutdown succeeded!" << std::endl;
					//shutdown_promise.set_value(); // Signal completion
				},
				[](const x::FlutterError& error) {
					//std::ofstream log_file("shutdown_log.txt", std::ios::app); // Open file in append mode
					//if (log_file.is_open()) {
					//	log_file << "Notify Shutdown failed: " << error.message() << std::endl; // Write error to file
					//	log_file.close(); // Close the file
					//}
					// This is the on_error callback
					 std::cerr << "Notify Shutdown failed: " << error.message() << std::endl;
					//shutdown_promise.set_value(); // Signal completion even on error
				}
			);
